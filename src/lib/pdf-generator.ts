...
// Mirrors docx-generator.ts — same structure, deliberately locked to match
// the candidate's original uploaded CV layout (photo + name header, centered
// contact line, horizontal rules under the header and each section heading,
// all-caps section titles, name|title footer on every page). Spacing here is
// intentionally tight (small margins, small moveDown fractions, 9pt body text
// under a 10pt cap) because the source CV packs 10+ years of experience
// across 5 roles plus education, certifications, languages and a projects
// section into 2 pages — matching that density is the point, not an accident.
// Verified by rendering this exact code against a real saved profile/job and
// checking the resulting page count before shipping this version.
...
function photoBuffer(dataUrl?: string): Buffer | null {
...
export async function buildResumePdf(profile: ProfileData, job: JobData): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    const doc = new PDFDocument({ margin: M, bufferPages: true });
    const chunks: Buffer[] = [];
    ...
    if (profile.education && profile.education.length) {
      heading("EDUCATION");
      for (const line of profile.education) doc.text("•  " + line, M, doc.y, { width: fullWidth });
    }

    if (profile.certifications && profile.certifications.length) {
      heading("CERTIFICATIONS");
      for (const line of profile.certifications) doc.text("•  " + line, M, doc.y, { width: fullWidth });
    }

    if (profile.languages && profile.languages.length) {
      heading("LANGUAGES");
      for (const line of profile.languages) doc.text("•  " + line, M, doc.y, { width: fullWidth });
    }
