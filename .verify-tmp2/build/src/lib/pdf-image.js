...
exports.extractLargestPhotoFromPdf = extractLargestPhotoFromPdf;
exports.photoBufferToDataUrl = photoBufferToDataUrl;
...
function extractLargestPhotoFromPdf(buffer) {
    ...
    while ((match = objRe.exec(text))) {
        ...
        if (!hasDCT)
            continue; // only handle JPEG XObjects, see note above
        ...
        try {
            if (hasFlate)
                raw = zlib_1.default.inflateSync(raw);
        }
        catch {
...
function photoBufferToDataUrl(buf) {
